...
// Flow Logs Endpoints
app.post('/api/flow-logs', async (req: any, res: any) => {
  try {
    const { execution_id, step, status, timestamp, data, site_name } = req.body;
    
    if (!execution_id || !step || !status) {
      return res.status(400).json({ error: 'execution_id, step, and status are required' });
    }

    // Insert or update execution
    const existingExecution = await db.sql`SELECT id FROM flow_executions WHERE execution_id = ${execution_id}`;
    
    if (!existingExecution[0]) {
      await db.sql`INSERT INTO flow_executions (execution_id, status, site_name) VALUES (${execution_id}, ${status}, ${site_name || 'Desconhecido'})`;
    } else {
      // Update status if it's a final state or if current is processing
      if (status === 'error' || status === 'success' || step === 'completed') {
         let finalStatus = status;
         if (step === 'completed' && status !== 'error') finalStatus = 'success';
         await db.sql`UPDATE flow_executions SET status = ${finalStatus}, updated_at = CURRENT_TIMESTAMP WHERE execution_id = ${execution_id}`;
      } else {
         await db.sql`UPDATE flow_executions SET updated_at = CURRENT_TIMESTAMP WHERE execution_id = ${execution_id}`;
      }
    }

    // If step is flowpost_response and we have a URL, update final_url
    if (step === 'flowpost_response' && data?.output?.url) {
      await db.sql`UPDATE flow_executions SET final_url = ${data.output.url} WHERE execution_id = ${execution_id}`;
    }

    // Insert log
    await db.sql`INSERT INTO flow_logs (execution_id, step, status, timestamp, data) VALUES (${execution_id}, ${step}, ${status}, ${timestamp || new Date().toISOString()}, ${JSON.stringify(data || {})})`;

    res.json({ success: true });
  } catch (error) {
    console.error('Error saving flow log:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});

app.get('/api/flow-executions', authenticateToken, async (req: any, res: any) => {
  try {
    const executions = await db.sql`SELECT * FROM flow_executions ORDER BY updated_at DESC LIMIT 100`;
    res.json(executions);
  } catch (error) {
...
app.get('/api/flow-executions/:execution_id/logs', authenticateToken, async (req: any, res: any) => {
  try {
    const logs = await db.sql`SELECT * FROM flow_logs WHERE execution_id = ${req.params.execution_id} ORDER BY timestamp ASC, id ASC`;
    res.json(logs);
  } catch (error) {
...
app.post('/api/analyze-link', async (req: any, res: any) => {
  ...
  try {
    ...
    if (requestUser && requestUser.gemini_api_key) {
      geminiApiKey = requestUser.gemini_api_key;
      console.log("Using personal API key from user settings");
    }

    // Fallback to global settings only if no personal key is set AND user is admin/produção or unauthenticated
    if (!geminiApiKey) {
      if (!requestUser || requestUser.role === 'admin' || requestUser.sector === 'Produção') {
        if (settingsMap.gemini_api_key) {
          geminiApiKey = settingsMap.gemini_api_key;
          console.log("Using API key from database settings");
        }
      } else {
        return res.status(403).json({ error: 'Você precisa configurar sua própria chave da API do Gemini nas Configurações para realizar análises. Isso evita sobrecarga no sistema.' });
      }
    }
